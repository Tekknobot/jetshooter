...
public class TargetCrit : MonoBehaviour
{
    ...
    public GameObject critTransform;
    ...
    public void critTakeDamage (float amount) {
        health -= amount;
        if (health <= 0f)
        {
            Death();
    ...
    {       
        ...
        Instantiate(explosionPrefab, critTransform.transform.position, Quaternion.identity);         
